...
def create_job(req: TrainRequest, db: Session = Depends(get_db)) -> JobOut:
    ds = crud.get_dataset(db, req.dataset_id)
    if ds is None:
    ...
    logger.info(f"jobs: created job id={job.id} dataset={req.dataset_id} task={task_type.value}")
...
def list_jobs(db: Session = Depends(get_db)) -> List[JobOut]:
    return [JobOut.model_validate(j) for j in crud.list_jobs(db)]
...
def list_models(job_id: int, db: Session = Depends(get_db)) -> List[ModelOut]:
    job = crud.get_job(db, job_id)
    if job is None:
        raise HTTPException(status_code=404, detail="job not found")
    return [ModelOut.model_validate(m) for m in crud.list_models_for_job(db, job_id)]


@router.get("/{job_id}/best", response_model=Optional[ModelOut])
def best_model(job_id: int, db: Session = Depends(get_db)) -> Optional[ModelOut]:
    job = crud.get_job(db, job_id)
    if job is None:
        raise HTTPException(status_code=404, detail="job not found")
    best = crud.best_model_for_job(db, job_id)
